...
<div class="container mt-5">
    ...
    <form action="${pageContext.request.contextPath}/hien-thi-san-pham-chi-tiet/update" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <select name="maSanPham" class="form-control" required>
                <c:forEach items="${listSanPham}" var="maSanPham">
                    <option value="${maSanPham.id}" label="${maSanPham.tenSanPham}" ${maSanPham.id == chiTietSanPham.maSanPham.id ? "selected":""}></option>
                </c:forEach>
            </select>
        </div>
        <div class="form-group">
            <label>Mau Sac:</label>
            <select name="maMauSac" class="form-control" required>
                <c:forEach items="${listMauSac}" var="maMauSac">
                    <option value="${maMauSac.id}" label="${maMauSac.tenMauSac}" ${maMauSac.id == chiTietSanPham.maMauSac.id ? "selected":""}></option>
                </c:forEach>
            </select>
        </div>
        <div class="form-group">
            <label>Sản Phẩm:</label>
            <select name="maKichThuoc" class="form-control" required>
                <c:forEach items="${listKichThuoc}" var="maKichThuoc">
                    <option value="${maKichThuoc.id}" label="${maKichThuoc.tenKichThuoc}" ${maKichThuoc.id == chiTietSanPham.maKichThuoc.id ? "selected":""}></option>
                </c:forEach>
            </select>
        </div>
        <div class="form-group">
            <label for="soLuong">Số Lượng:</label>
            <input type="number" id="soLuong" name="soLuong" class="form-control" value="${chiTietSanPham.soLuong}" required>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="file" id="hinhAnh" name="hinhAnh" class="form-control" value="${chiTietSanPham.hinhAnh}" required>
        </div>
        <div class="form-group">

...
<div class="container mt-5">
    ...
    <form action="/hien-thi-hoa-don-chi-tiet/add" method="post" class="bg-light p-4 rounded shadow-sm">
        ...
        <div class="form-group">
            ...
            <select id="maSanPhamChiTiet" name="maSanPhamChiTiet" class="form-control" onchange="document.getElementById('gia').value=this.options[this.selectedIndex].getAttribute('data-gia');">
                <c:forEach var="sanPhamChiTiet" items="${listSanPhamChiTiet}">
                    <option value="${sanPhamChiTiet.id}" data-gia="${sanPhamChiTiet.gia}">${sanPhamChiTiet.maSanPham.tenSanPham}</option>
                </c:forEach>
            </select>
        </div>
